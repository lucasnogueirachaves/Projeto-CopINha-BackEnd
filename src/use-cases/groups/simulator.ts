...
export class SimulatorUseCase {
  ...
  async execute({
    ...
    for (const match of group.matches) {
      ...
      const simulation = simulatedMatches.find(
        (simulatedMatch) => simulatedMatch.publicId === match.publicId,
      )
  ...
  private calculateMatch(
    ...
    const homeTeam = classification.find((team) => team.teamId === homeTeamId)

    const awayTeam = classification.find((team) => team.teamId === awayTeamId)
